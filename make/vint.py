...
unsigned vblen16(unsigned short x);
unsigned vblen32(unsigned       x);
unsigned vblen64(uint64_t       x);

// ---- Length of compressed value. Input in is the first char of the compressed buffer start (Ex. vbvlen32(in[0]) )
unsigned vbvlen16(unsigned x);
unsigned vbvlen32(unsigned x);
unsigned vbvlen64(unsigned x);
