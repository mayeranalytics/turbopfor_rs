...
def make(algo, algo_name, fn_cat, fn_cat_desc, codec, size, typ):
    ...
    if codec in ['enc', 'pack']:
        ...
        fn_def = f"""\
            ...
            /// # Arguments
            /// * `input` - `Vec<u{size}>` containing the uncompressed input
            /// * `output` - `Buffer<u8>` containing the compressed output
            /// # Returns
            /// Number of bytes written to output
            pub fn {rs_fn_name}(input: &Vec<{input_type}>, output: &mut Buffer<{output_type}>) -> usize
            {{
                unsafe {{
                    let n_bytes_written = ic::{ic_fn_name}(input.as_ptr(), input.len(), output.as_mut_ptr());
        ...
        fn_def = f"""\
            ...
            /// * `output` - `Buffer<{output_type}>` containing the decompressed output
            /// # Returns
            /// Number of bytes read from input
            pub fn {rs_fn_name}(input: &mut Buffer<{input_type}>, output_len: usize, output: &mut Buffer<{output_type}>) -> usize
            {{
                unsafe {{
                    let n_bytes_read = ic::{ic_fn_name}(input.as_ptr(), output_len, output.as_mut_ptr());
